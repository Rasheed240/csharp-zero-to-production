// 04-server-gc.cs — The SAME workload under SERVER GC.
//
// Run:  dotnet run 04-server-gc.cs -c Release
//
// A .NET 10 file-based app can set MSBuild properties with #:property lines, so
// the GC mode is switchable without a .csproj. Everything below the header is
// byte-identical to 03-workstation-gc.cs; diff them and confirm it.

#:property ServerGarbageCollection=true
#:property ConcurrentGarbageCollection=true
